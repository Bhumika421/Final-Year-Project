...
require_once __DIR__ . '/../src/controllers/message_controller.php';
...
$router->add('GET',  '/api/auth/me',              function() { $user = require_auth(); auth_me($user); });
$router->add('PUT',  '/api/auth/update',          function() { $user = require_auth(); auth_update($user); });
...
$router->add('GET',    '/api/admin/users',                function() { $admin = require_admin(); admin_users_list(); });
$router->add('PUT',    '/api/admin/users/{id}',           function($p) { $admin = require_admin(); admin_user_update($p); });
...
// Upload
$router->add('POST', '/api/upload/images', function() { $agency = require_agency(); upload_images(); });
...
$router->add('POST', '/api/messages',          function() { $user = require_auth(); messages_send($user); });
$router->add('GET',  '/api/messages',          function() { $user = require_auth(); messages_get_conversation($user); });
$router->add('GET',  '/api/messages/threads',  function() { $user = require_auth(); messages_get_threads($user); });
$router->add('GET',  '/api/messages/unread',   function() { $user = require_auth(); messages_unread_count($user); });
