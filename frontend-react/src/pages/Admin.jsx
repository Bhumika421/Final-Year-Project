...
export default function Admin() {
  ...
  async function load() {
    ...
    try {
      ...
      setTours(toursData); setPendingTours(pendingToursData); setPendingAgencies(pendingAgenciesData);
      setTickets(ticketsData); setBookings(bookingsData); setUsers(usersData);
      setStats({
        totalUsers: usersData.length,
        activeTours: toursData.filter(t => t.approval_status === 'approved').length,
        totalBookings: bookingsData.length,
        totalRevenue: bookingsData.reduce((sum, b) => sum + (parseFloat(b.total_price) || parseFloat(b.amount) || 0), 0)
      });
          ...
          <div style={{padding:'32px'}}>
            ...
            <div style={{background:'linear-gradient(180deg, rgba(18,26,45,0.98), rgba(11,18,32,0.96))', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'14px', padding:'20px'}}>
              <table style={{width:'100%', borderCollapse:'collapse', fontSize:'13px'}}>
                ...
                <tbody>
                  {users.length === 0 ? (
                    <tr><td colSpan="6" style={{textAlign:'center', padding:'30px', color:'rgba(232,238,252,0.4)'}}>No users found</td></tr>
                  ) : users.map(u => (
                  ...
                  <div key={b.id} style={{padding:'12px', border:'1px solid rgba(255,255,255,0.05)', borderRadius:'10px', marginBottom:'8px'}}>
                    <div style={{display:'flex', alignItems:'center', justifyContent:'space-between'}}>
                      <div>
                        ...
                        <p style={{margin:0, fontSize:'13px', color:'rgba(232,238,252,0.75)'}}>Total: ${Number(b.total_price||b.amount||0).toFixed(2)} • Status: {b.status}</p>
                      </div>
